...
void uArmController::init()
{
    ...
    mServo[SERVO_ROT_NUM].setPulseWidthRange(900, 2100);
    mServo[SERVO_LEFT_NUM].setPulseWidthRange(900, 2100);
    mServo[SERVO_RIGHT_NUM].setPulseWidthRange(900, 2100);
...
void uArmController::writeServoAngle(byte servoNum, double servoAngle, boolean writeWithOffset )
{
	...
	if ( (servoNum == SERVO_LEFT_NUM) ) {
		float out = map(servoAngle,0,180,5, 275);
		mServo[servoNum].write(out, 20);
	//	if(servoNum == SERVO_LEFT_NUM) Serial.println(out);
	}
	else if ((servoNum == SERVO_RIGHT_NUM)) {
		float out = map(servoAngle, 0, 180, -40, 320);
		mServo[servoNum].write(out, 20);
		//if (servoNum == SERVO_RIGHT_NUM) Serial.println(2 * (180 - servoAngle));
	}
	else 
		mServo[servoNum].write(servoAngle, 20);
	//if (servoNum == SERVO_RIGHT_NUM) Serial.println(servoAngle);
}
...
unsigned char uArmController::xyzToAngle(double x, double y, double z, double& angleRot, double& angleLeft, double& angleRight, boolean allowApproximate)
{
	...
	//angleRight = map(angleRight, 0, 180, -40, 320);
	//angleLeft = map(angleLeft, 0, 180, 5, 275);
...
unsigned char uArmController::limitRange(double& angleRot, double& angleLeft, double& angleRight)
{
    ...
    angleLeft = constrain(angleLeft, 0.00, 275.00);
    angleRight = constrain(angleRight, 0.00, 320.00);
