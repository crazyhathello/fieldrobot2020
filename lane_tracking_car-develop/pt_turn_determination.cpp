...
int turn_determination(Mat crop)
{
	...
	Scalar blue_low_bound(100, 50, 50), blue_high_bound(135,255, 255); 

	Mat clustered_img, hsv_img, edge_clst, blue_object, target, knn_hsv_mix, hsv_channels[3];
	Mat element3x3 = getStructuringElement(MORPH_RECT, Size(3, 3), Point(-1, -1));
	vector<Vec4i> lines;
...
//	dilate(edge_clst, edge_clst, element3x3, Point(-1, -1), 1);
//	morphologyEx(edge_clst, edge_clst, MORPH_OPEN, element3x3, Point(-1, -1), 2);
	HoughLinesP(edge_clst, lines, 0.7, CV_PI/90, 40, 30, 20);
/*
...
*/		
	...
	if(abs(r_slope) >= 10 || isinf(r_slope)) r_slope = 10;
	if(abs(l_slope) <= -10 || isinf(l_slope)) l_slope = -10;
	cout << "r: " << r_slope << endl;
	cout << "l: " << l_slope << endl;
...
//	namedWindow("blue", CV_WINDOW_NORMAL);
	imshow("imgOriginal", crop);
	imshow("imgKmeans", edge_clst);
	imshow("imgEdge", clustered_img);
